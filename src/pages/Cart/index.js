...
const Cart = () => {
  ...
  const handleUpdateQuantity = async (id, newQuantity) => {
    ...
    } else await updateCartItem(id, newQuantity);
    fetchCart();
  };
    ...
    <>
      ...
      <div className={styles.container}>
          ...
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item) => (
              <div key={item.id} className={styles.cart_item}>
                <input type="checkbox" />
                <img src={item?.productImage || product} alt="Product" />

                <div className={styles.item_info}>
                  <p className={styles.brand}>{item.product.code}</p>
                  <p className={styles.name}>{item.product.name}</p>
                  <span className={styles.variant}>
                    Phân loại: {item.product.capacity} ml
                  </span>
                ...
                <div className={styles.item_quantity}>
                  ...
                  <input
                    ...
                    value={item.quantity}
                    onChange={(e) =>
                      handleChangeQuantity(item.product.id, e.target.value)
                    }
                  />
                  <button
                    onClick={() =>
        ...
        <div className={styles.cart_right}>
          ...
          <button className={styles.checkout_btn} onClick={() => handleOrder()}>
            Tiến Hành Đặt Hàng
          </button>
        </div>
      </div>
